...
Every launch carries a route: level, target, flags. Default `high`; `max` asks first, and `ultra` is the user's to type. The fork runs from **this session, never from a subagent**; nothing after the launch happens until its report is in. One run reports at most 15 findings.
...
- **ultra** — offered on any max predicate, recommended at `files ≥ 20`.

A field the task's own question settled is settled here — granted ⇒ `max`, declined ⇒ `high`.

Details: references/contract-scan-and-levels.md — read for the scan calls, the card fields and the table.
...
Details: references/target-flags-and-asking.md — read before picking a target or a flag.
...
4. Emit the `review-route:` line before any launch.
5. Launch from the session: `Skill(skill: "code-review", args: "<level> <target> <flags>")`, then wait for its report.
6. Re-run after fixes: the `prior` row decides it. A shape change ⇒ one more scan, one more question at most.

Details: references/procedure-audit-and-excuses.md — read for the route-line format and the option wording.
